<template>
  <ebook-dialog :title="title" ref="dialog">
    <div
      ...
      v-if="!ifNewGroup && !showNewGroup && !haveConfirmTitle"
    >
      <div
    ...
    <div
      class="dialog-new-group-wrapper"
      v-else-if="ifNewGroup || showNewGroup"
    >
    ...
    >
      ...
      <div
        ...
        :class="{
          'is-empty': !newGroupName || newGroupName.trim().length === 0,
        }"
        v-if="ifNewGroup || showNewGroup"
      >
        {{ $t("shelf.confirm") }}
      </div>
    </div>
    <div slot="btn" class="group-dialog-btn-wrapper" v-else>
      <div class="dialog-btn" @click="hide">{{ $t("shelf.cancel") }}</div>
      <div class="dialog-btn" @click="clearCache">
        {{ $t("shelf.confirm") }}
      </div>
    </div>
  </ebook-dialog>
...
import { saveBookShelf, clearLocalStorage } from "../../utils/localStorage";
...
export default {
  ...
  computed: {
    ...
    title() {
      return this.haveConfirmTitle ? this.$t("shelf.clearConfirm")
        : !this.ifNewGroup ? this.$t("shelf.moveBook")
        : this.changeGroupName ? this.$t("shelf.editGroupName")
  ...
  methods: {
    ...
    createNewGroup() {
      ...
      if (this.showNewGroup) {
        ...
        this.simpleToast(this.$t("shelf.changeGroupNameSuccess"));
        this.onComplete();
      } else {
        const group = {
